...
async function encryptPayload(data) {
  ...
  try {
    ...
    if (data.chunk && (data.chunk instanceof ArrayBuffer || data.chunk instanceof Uint8Array)) {
      ...
      let binary = '';
      // Use a more efficient chunked approach for large buffers to avoid stack issues here too
...
function configurePlatformMode() {
  ...
  state.webJoinOnly = false; // website now has full P2P; this flag is no longer used for gating
...
async function sendFileToPeer(connection, file, options = {}) {
  ...
  const started = await safeSendToConnection(connection, {
    type: 'file-start',
  ...
  for (let index = 0; index < totalChunks; index += 1) {
    ...
    const sent = await safeSendToConnection(connection, {
      type: 'file-chunk',
  ...
  await safeSendToConnection(connection, {
    type: 'file-end',
